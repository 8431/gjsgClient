...
cc.Class({
    ...
    initZhuangbei: function (res) {
        ...
        self.LvLabel.string = msg.level;
         ...
         let icon = cc.find('Canvas/sprite/icon');
        let sprite2 = icon.getComponent('cc.Sprite');
            //网络加载图片
        ...
        for (let i = 0; i < bsArr.length; i++) {
            if (null == bsArr[i]) {
                cc.loader.loadRes('prefabs/kong2', cc.Prefab, function (err, prefab) {
                    if (!err) {
                        cc.log(bsArr[i])
                        let kong = cc.instantiate(prefab);
