cc.Class({
    ...
    init: function (index, data) {
        ...
        this.zhanBtn.on("click", function (event) {
            ajaxUtils(HxsgUrl.setFjState, { 'id': this.data.fuId }, function (msg) {
                showAlert(msg, function () { loadingScene(HxsgScenes.fujiang, HxsgScenes.index) }, function () { loadingScene(HxsgScenes.fujiang, HxsgScenes.index) })
            });
        }, this)
    ...
    zhanBtnEvent: function () {

        cc.log(this.node.name + ' 出战');
    },

    xiuBtnEvent: function () {
